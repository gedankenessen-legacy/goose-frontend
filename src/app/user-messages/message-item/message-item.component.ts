...
export class MessageItemComponent extends SubscriptionWrapper implements OnInit {
  ...
  ngOnInit(): void {
    this.subscribe(this.getTicket(this.message.projectId, this.message.issueId));
  }
  ...
  getTicket(projectId: string, issueId: string): Observable<Issue> {
    return this.issueService.getIssue(projectId, issueId).pipe(tap(issue => this.issueName = issue.issueDetail.name))
  }
}
